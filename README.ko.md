...
`:watch` 스크립트는 경로 계획이 되는 셰어웨어 맵 전부에 있습니다. `e1m1` 부터 `e1m6` 까지와 `e1m9` 입니다. E1M7 과 E1M8 은 아직 출구 경로를 찾지 못해 스크립트를 두지 않았습니다. 측정을 마친 것은 E1M1 부터 E1M3 까지이고, 나머지는 구경용이지 클리어를 기대할 대상이 아닙니다.

엔진은 정확 틱 스텝 사이의 매 프레임마다 "Pause" 배너를 그립니다. 그게 대부분의 프레임이라 기본으로 숨깁니다. 보시려면 `--no-hide-pause` 를 붙이세요. 스테이지 WAD 에 투명 패치를 끼워 덮는 방식이라 렌더링만 바뀌고 시뮬레이션은 그 lump 를 읽지 않습니다.
